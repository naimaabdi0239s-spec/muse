...
const faqItems = [
  {
    ...
    answer: "We design custom websites tailored to your budget and needs. Pricing depends on scope, complexity, and features. Rather than one-size-fits-all pricing, we discuss your vision first to create a transparent quote. Book a discovery call to get started with a consultation."
  },
  {
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-[color:var(--cream)]/90 border-b border-[color:var(--maroon)]/15">
        <div className="mx-auto max-w-6xl px-6 h-20 flex items-center justify-between">
          ...
          <a
            ...
            onClick={() => track("cta_book_discovery_call")}
      ...
      <main>
        ...
        <section id="services" className="mx-auto max-w-6xl px-6 py-14 animate-fade-in">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {services.map((s) => (
              <div key={s.title} className="bg-beige rounded-xl p-8 border-l-4" style={{ borderLeftColor: "var(--maroon)" }}>
          ...
          <p className="mt-10 text-sm text-coffee/80">
            Have something else in mind? <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="text-maroon hover:underline underline-offset-4">Let's talk about custom work →</a>
          </p>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 py-14">
          ...
          <div className="flex flex-wrap items-end justify-between gap-4 mb-6">
            ...
            <a href={featured.liveUrl} target="_blank" rel="noopener noreferrer" className="rounded-full border border-maroon text-maroon px-5 py-2 text-xs uppercase tracking-widest hover:bg-maroon hover:text-cream transition-colors inline-flex items-center gap-2">
          ...
          {beforeImage && afterImage && (
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <img src={beforeImage.src} alt={beforeImage.alt} loading="lazy" width={1600} height={1200} className="w-full aspect-[4/3] object-contain rounded-lg border border-[color:var(--maroon)]/20 bg-beige" />
                <p className="mt-3 text-xs uppercase tracking-widest text-[color:var(--maroon)] font-medium text-center">Before</p>
              </div>
              <div>
                <img src={afterImage.src} alt={afterImage.alt} loading="lazy" width={1600} height={1200} className="w-full aspect-[4/3] object-contain rounded-lg border border-[color:var(--maroon)]/20 bg-beige" />
        ...
        <section id="work" className="mx-auto max-w-6xl px-6 py-14">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {caseStudies.map((w) => (
              <div key={w.slug} className="group bg-beige rounded-xl overflow-hidden flex flex-col">
                <div className="aspect-[4/3] overflow-hidden bg-cream p-5 flex items-center justify-center">
                  <img src={w.cover} alt={`${w.title} website preview`} loading="lazy" width={1200} height={1500} className="w-full h-full object-contain rounded-lg transition-transform duration-500" />
                </div>
                <div className="p-6">
                ...
                <div className="px-6 pb-6">
                  <a href={w.liveUrl} target="_blank" rel="noopener noreferrer" className="text-sm text-maroon hover:underline underline-offset-4 inline-flex items-center gap-2">
                    View live project <ArrowUpRight className="w-3.5 h-3.5" />
                  </a>
                </div>
              </div>
        ...
        <section id="faq" className="mx-auto max-w-6xl px-6 py-14">
          <div className="mb-12">
            ...
            <h2 className="font-serif text-4xl md:text-5xl text-center mb-8">Frequently Asked Questions</h2>
            <p className="text-center text-coffee/80 max-w-2xl mx-auto">
              Find answers to questions about our services, pricing, and process. Can't find what you're looking for? <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" className="text-maroon hover:underline">Book a discovery call</a>.
            </p>
          </div>
          <FAQAccordion />
        </section>
        ...
        <section id="about" className="mx-auto max-w-6xl px-6 py-8 grid md:grid-cols-2 gap-8 items-center">
          <div>
            ...
            <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" onClick={() => track("cta_book_discovery_call_about")} className="mt-6 inline-flex items-center gap-2 rounded-full px-6 py-3 text-sm text-cream transition-colors hover:opacity-90" style={{ background: "var(--coffee)" }}>
          ...
          <div className="flex justify-center md:justify-start">
            <div className="w-full max-w-lg aspect-square rounded-xl overflow-hidden bg-beige border border-[color:var(--maroon)]/20">
              <img src="/aboutus.jpg" alt="Designer reviewing brand work on a phone beside an iMac showing a color-system layout" loading="lazy" width={1200} height={1200} className="w-full h-full object-cover" />
            </div>
          </div>
        </section>
        ...
        <section className="mx-auto max-w-6xl px-6 py-14">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <figure key={t.name} className="relative bg-beige rounded-2xl p-8 pt-10 shadow-sm border border-[color:var(--maroon)]/20 animate-testimonial-slide" style={{ animationDelay: `${i * 0.3}s` }}>
                <div className="absolute -top-4 left-6 bg-cream rounded-full p-2 border border-[color:var(--maroon)]/20"><Quote className="w-4 h-4 text-maroon" /></div>
                <blockquote className="font-serif text-chocolate text-lg leading-snug italic">"{t.quote}"</blockquote>
        ...
        <div className="mx-auto max-w-6xl px-6"><div className="border-t border-[color:var(--maroon)]/20" /></div>
        <section id="contact" className="mx-auto max-w-6xl px-6 py-14 grid md:grid-cols-2 gap-6">
          <div className="rounded-xl p-10 border-l-4 flex flex-col justify-between" style={{ background: "var(--chocolate)", borderLeftColor: "var(--maroon)", color: "var(--cream)" }}>
            <div>
              <h3 className="font-serif text-3xl mb-4" style={{ color: "var(--cream)" }}>Have a vision in mind?</h3>
              <p className="text-sm opacity-80 leading-relaxed max-w-sm">Ready to talk about your project? Book a discovery call and let's figure out the right direction together.</p>
            </div>
            <a href={BOOKING_URL} target="_blank" rel="noopener noreferrer" onClick={() => track("cta_book_discovery_call_contact")} className="mt-8 inline-flex items-center gap-2 text-sm uppercase tracking-widest" style={{ color: "var(--cream)" }}>
              Book a discovery call <ArrowRight className="w-4 h-4" />
            </a>
          </div>
          <div className="rounded-xl p-10 border-l-4 bg-beige" style={{ borderLeftColor: "var(--maroon)" }}>
            <h3 className="font-serif text-3xl mb-6">Connect with us</h3>
            <ul className="space-y-3 text-sm">
              <li><a href="https://www.instagram.com/musewebstudio/" target="_blank" rel="noopener noreferrer" onClick={() => track("contact_instagram_click")} className="inline-flex items-center gap-3 text-coffee hover:text-maroon transition-colors"><Instagram className="w-4 h-4" /> Instagram</a></li>
              <li><a href="https://www.facebook.com/profile.php?id=61592306638218" target="_blank" rel="noopener noreferrer" onClick={() => track("contact_facebook_click")} className="inline-flex items-center gap-3 text-coffee hover:text-maroon transition-colors"><Facebook className="w-4 h-4" /> Facebook</a></li>
              <li><a href="mailto:musewebsitestudio@gmail.com" onClick={() => track("contact_email_click")} className="inline-flex items-center gap-3 text-coffee hover:text-maroon transition-colors"><Mail className="w-4 h-4" /> musewebsitestudio@gmail.com</a></li>
            </ul>
          </div>
        </section>
      </main>
      ...
      <footer className="mt-10" style={{ borderTop: "2px solid var(--maroon)" }}>
        <div className="mx-auto max-w-6xl px-6 py-14 grid md:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <p className="mt-3 text-sm text-coffee/70 max-w-xs">{geoOptimizedSummary}</p>
          </div>
